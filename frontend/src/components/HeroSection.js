  ...
  return (
    <section className="bg-white">
      <div className="container mx-auto py-10 px-4">
            <div className="bg-blue-500 py-16 rounded">
        <div className="container mx-auto px-4">
          <h1 className="text-4xl font-bold text-white mb-6">Welcome to the Hero Section</h1>
      ...
      <div className="shadow-xl mt-8 mr-0 mb-0 ml-0 pt-4 pr-10 pb-4 pl-10 flow-root rounded-lg sm:py-2">
        <div className="pt--10 pr-0 pb-10 pl-0">
          ...
          <p className="text-sm mt-1 mr-0 mb-0 ml-0 font-semi-bold text-gray-500">Lorem ipsum dolor sit amet, consectetur adipis</p>
        </div>
              {sortedJobs.length > 0 ? (
                sortedJobs.slice(0, 10).map((job) => (
                  <div key={job.slug} className="pt-5 pr-0 pb-0 pl-0 mt-5 mr-0 mb-0 ml-0">
                    <div className="sm:flex sm:items-center sm:justify-between sm:space-x-5 rounded shadow-sm">
                      <div className="flex items-center flex-1 min-w-0">
                        <img src='https://v1.tailwindcss.com/_next/static/media/tailwind-ui-sidebar.2ccd3a8ec5f31f428204b5c3c4d9a485.png' className="flex-shrink-0 object-cover rounded-full btn- w-10 h-10" />
                        <div className="mt-0 mr-0 mb-0 ml-4 flex-1 min-w-0">
                          <p className="text-lg font-bold text-gray-800 truncate">{job.title}</p>
                          <p className="text-gray-600 text-md">{job.company_name}</p>
                        </div>
                      </div>
                      <div className="mt-4 mr-0 mb-0 ml-0 pt-0 pr-0 pb-0 pl-14 flex items-center sm:space-x-6 sm:pl-0 sm:mt-0">
                        <button onClick={handleApplyNow} className="bg-gray-800 pt-2 pr-6 pb-2 pl-6 text-lg font-medium text-gray-100 transition-all duration-200 hover:bg-gray-700 rounded-lg">Apply</button>
                      </div>
                    </div>
                  </div>
                ))
